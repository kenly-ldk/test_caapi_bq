...
: "${LOCATION:=global}"
: "${IDENTITY_MODE:=adc}"
: "${AGENT_ID:?set AGENT_ID in config.env (create it once via ensure_agent.py)}"

ENV_VARS="BILLING_PROJECT=${BILLING_PROJECT},LOCATION=${LOCATION},AGENT_ID=${AGENT_ID},IDENTITY_MODE=${IDENTITY_MODE}"
if [[ -n "${TARGET_SA:-}" ]]; then
